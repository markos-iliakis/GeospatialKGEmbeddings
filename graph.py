...
class Formula:
    ...
    def __init__(self, query_type, rels):
        ...
        if query_type in ["1-chain", "2-chain", "3-chain"]:
            ...
            self.anchor_types = (rels[-1][-1],)

        # o->t<-o      o->t<-o
        #                 ^
        #                 |
        #                 o
        elif query_type in ["2-inter", "3-inter"]:
            self.anchor_types = tuple([rel[-1] for rel in rels])
        ...
        #        o
            ...
            self.anchor_types = (rels[0][-1], rels[1][-1][-1])
        ...
        #     o->o<-o
            ...
            self.anchor_types = (rels[1][0][-1], rels[1][1][-1])
...
class Query:
    ...
    def __init__(self, query_graph, neg_samples, hard_neg_samples, neg_sample_max=100, keep_graph=False):
        ...
        query_graph: Just the 1st item in each entry of train/val/test_edges and train/val/test_queries_2/3
            ('1-chain', (1326, (head_type, relation, tail_type), 8451))
    ...
    def reverse_edge(self, edge):
        ...
        edge: (359, (head_type, relation, tail_type), 273)
        """
        return edge[-1], self.reverse_relation(edge[1]), edge[0]

    def reverse_relation(self, relation):
        """
        reverse the triple template
            relation: (head_type, relation, tail_type)
        """
        return relation[-1], relation[1], relation[0]

    def contains_edge(self, edge):
        """
        Given an edge structure, decide if it is in the current query_graph
        edge: (359, (head_type, relation, tail_type), 273)
        """
        ...
        return edge in edges or self.reverse_edge(edge) in edges

    def get_edges(self):
        """
        Given the current query_graph, get a list of edge structures and their reverse edge
        return a set of these edge structure
        """
        edges = self.query_graph[1:]
        if "inter_chain" in self.query_graph[0] or "chain_inter" in self.query_graph[0]:
            edges = (edges[0], edges[1][0], edges[1][1])
        return set(edges).union(set([self.reverse_edge(e) for e in edges]))
    ...
    def deserialize(serial_info, keep_graph=False):
        """
        Given an entry (serial_info) in train/val/test_edges and train/val/test_queries_2/3
...
class Graph:
    ...
    def __init__(self, features, feature_dims, relations, adj_lists, rid2inverse, id2type):
        ...
        Args:
            features(nodes, type): a embedding lookup function to make a dict() from node type to EmbeddingBag
                nodes: a lists of global node id which are in type
                type: node type
            ...
            adj_lists: a dict about the edges in KG (note that, they already add all reverse edges)
                key: triple template, i.e. (head_type, relation, tail_type)
                value: a default-dict about all the edges instance of this metapath
    ...
    def remove_edges(self, edge_list):
        ...
        Args:
            edge_list: a list of edges, like (122939, (head_type, relation, tail_type), 107943)
        """
    ...
    def get_negative_edge_samples(self, edge, num, rejection_sample=True):
        ...
        the negative nodes has the same node type as head id but does not satisfy the edge
        Args:
        ...
        if rejection_sample:
            ...
            while len(neg_nodes) < num:
                ...
                if neg_node not in self.adj_lists[self._reverse_relation(edge[1])][edge[2]]:
    ...
    def sample_test_queries(self, train_graph, q_types, samples_per_type, neg_sample_max, verbose=True, id2geo=None):
        ...
        Args:
            ...
            id2geo: node id => [longitude, latitude], if not None, we simply query with target node has coordinate
        Return:
    ...
    def sample_queries(self, arity, num_samples, neg_sample_max, verbose=True, id2geo=None):
        ...
        Args:
            ...
            num_samples: number of sampled query for this arity
            neg_sample_max: the maximum negative samples
            verbose: whether to print the query sampling number
    ...
    def get_negative_samples(self, query, id2geo=None):
        ...
        Return:
            neg_samples: a set of nodes whose with the target node type, but do not satisfy the current query
            hard_neg_samples: a set of nodes whose with the target node type, also satisfy one or more edge, but do not satisfy the current whole query
    ...
    def sample_query_subgraph_bytype(self, q_type, start_node=None):
        ...
        Args:
            q_type: query type
            start_node: a tuple, (target node id, target node type)
        Return:
            query: a tuple, (query_type, edge1, edge2, ...), for 3-inter_chain and 3-chain_inter, the 3rd item is a tuple of two edges
        """
    ...
    def sample_inter_query_subgraph(self, arity, start_node=None, possible_node_list=[], id2geo=None):
        """
        Given arity, and a start_node (target node id, target node type), sample a query from the adj_lists
        Similar to sample_query_subgraph(), but here, we only sample inter query which is equivalent to sample a node's neighborhood
        Args:
            arity: the number of edge in the query to be sampled
            start_node: a tuple, (target node id, target node type)
    ...
    def get_inter_query_negative_samples(self, query, id2geo=None):
        """
        Given an inter query, get the negative samples and hard negative samples for the target node
        if id2geo is not None:
    ...
    def get_metapath_neighs(self, node, rels):
        ...
        Args:
            node: a center node id (anchor node)
